  ...
  return (
    <main className="relative z-10 pt-20 pb-12 space-y-24 md:space-y-32 overflow-hidden">
                  ...
                  {sortedNews
                    ...
                    .map((item, i) => (
                      <Link key={item.id} href={`/article/${item.slug}`} className="flex gap-5 group items-start">
                        ...
                        <div>
                          <span className="text-[10px] font-bold text-cyan-600 dark:text-cyan-400 uppercase tracking-widest block mb-1.5">{item.category}</span>
                ...
                <div className="space-y-8 relative border-l border-border pl-8 ml-2.5">
                  {latestNews.slice(0, 4).map((item) => (
                    <Link key={item.id} href={`/article/${item.slug}`} className="block group relative">
                      ...
                      <span className="text-[10px] text-muted-foreground font-mono mb-1 block">
      ...
      <section id="cerita-list" className="bg-muted/30 py-20 md:py-32">
        <div className="container px-4 mx-auto">
            ...
            <div className="flex items-end justify-between mb-16 flex-wrap gap-4">
                <div className="space-y-2">
                    <span className="text-amber-600 dark:text-amber-500 font-bold tracking-widest text-xs uppercase">Kearifan Lokal</span>
                    <h2 className="text-4xl md:text-5xl lg:text-6xl font-serif font-bold text-foreground">Cerita <span className="text-amber-600 dark:text-amber-500 italic">Rakyat</span></h2>
                </div>
                <Link href="/category/cerita" className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-muted-foreground hover:text-amber-600 dark:hover:text-amber-500 transition-colors group">
                Lihat Semua <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
                </Link>
            </div>
            </FadeIn>
            ...
            {ceritaArticles.slice(0, 4).map((item, i) => (
                ...
                <Link href={`/article/${item.slug}`} className="block group h-full">
                    ...
                    <div className="absolute bottom-6 left-6 right-6">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-amber-400 mb-2 block backdrop-blur-md bg-black/20 w-fit px-2 py-1 rounded-md">Cerita</span>
      ...
      <section id="sosok" className="relative py-12 md:py-24 overflow-hidden">
        <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-purple-100 dark:bg-purple-900/10 rounded-full blur-3xl -z-10 opacity-50 pointer-events-none translate-x-1/3 -translate-y-1/4"></div>
        
        <div className="container px-4 mx-auto">
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-24 items-center">
            <FadeIn className="space-y-10">
                <div className="space-y-4">
                    <span className="inline-block px-3 py-1 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded-full text-xs font-bold uppercase tracking-widest border border-purple-200 dark:border-purple-800">Inspirasi Lokal</span>
                    <h2 className="text-5xl lg:text-7xl font-serif font-bold leading-[0.9]">Wajah <br /> <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600">Banjarnegara.</span></h2>
                ...
                {sosokArticles.slice(0, 3).map((sosok, i) => (
                    ...
                    <div>
                        <h4 className="text-xl font-bold group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors line-clamp-1 mb-1">{sosok.title}</h4>
                        <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground block mb-2">Tokoh Inspiratif</span>
                ...
                <Link href="/category/sosok-inspiratif" className="inline-flex items-center gap-2 px-8 py-4 bg-foreground text-background rounded-full font-bold text-xs uppercase tracking-widest hover:bg-purple-600 hover:text-white transition-all duration-300">
                Lihat Semua Tokoh <ArrowRight size={14} />
                </Link>

            </FadeIn>
            
            <FadeIn delay={200} className="relative h-[600px] rounded-[3rem] overflow-hidden hidden lg:block group shadow-2xl shadow-purple-900/10">
                <div className="absolute inset-0 bg-purple-900/20 group-hover:bg-transparent transition-all z-10 duration-700"></div>
                <Image
      ...
      <section id="potensi" className="container px-4 mx-auto pb-24">
        <FadeIn>
          <div className="flex items-center justify-between mb-12 flex-wrap gap-4 border-b border-border pb-6">
            <h2 className="text-3xl lg:text-5xl font-serif font-bold text-foreground">Potensi <span className="text-emerald-600 dark:text-emerald-500 italic">&</span> Unggulan</h2>
            <Link href="/category/potensi" className="text-xs font-bold uppercase tracking-widest text-muted-foreground hover:text-emerald-600 dark:hover:text-emerald-500 transition-colors">Lihat Semua</Link>
          </div>
        </FadeIn>

        <div className="grid lg:grid-cols-4 lg:grid-rows-2 gap-6 h-auto lg:h-[600px]">
          <FadeIn className="lg:col-span-2 lg:row-span-2 bg-emerald-950 rounded-[2.5rem] p-8 lg:p-12 relative overflow-hidden group border border-emerald-900 lg:min-h-0 min-h-[400px]">
            {potensiArticles[0] && (
              <>
                <Image
                ...
                <div className="relative z-10 flex flex-col justify-between h-full space-y-12">
                  <span className="inline-block px-4 py-2 bg-emerald-500/20 backdrop-blur text-emerald-300 rounded-lg text-xs font-bold uppercase tracking-widest w-fit border border-emerald-500/30">
                    Komoditas Unggulan
                  </span>
                  <div>
                    ...
                    <p className="text-emerald-100/80 max-w-sm mb-8 line-clamp-2 text-lg">{potensiArticles[0].summary}</p>
                    <Link href={`/article/${potensiArticles[0].slug}`} className="bg-white text-emerald-950 px-8 py-4 rounded-full font-bold uppercase tracking-widest hover:bg-emerald-400 transition-colors inline-block text-xs shadow-lg shadow-emerald-900/50">
                      Baca Selengkapnya
                    </Link>
                  </div>
                </div>
          ...
          <FadeIn delay={200} className="lg:col-span-1 bg-amber-50 dark:bg-amber-950/30 border border-amber-100 dark:border-amber-900/50 text-foreground rounded-[2.5rem] p-8 flex flex-col justify-between min-h-[200px]">
            <Quote size={40} className="text-amber-500/30" />
            <p className="text-lg font-serif italic leading-tight text-amber-900 dark:text-amber-100">&quot;Keramik Klampok adalah warisan seni yang tak ternilai harganya.&quot;</p>
          </FadeIn>

          <FadeIn delay={300} className="lg:col-span-2 bg-foreground text-background rounded-[2.5rem] p-8 lg:p-10 flex items-center justify-between relative overflow-hidden group min-h-[200px]">
